...
function Show_Product() {
  const { user } = useAuth();
  const u_id = user?.id;
  const { date, user_id, order_id } = useParams();
  ...
  useEffect(() => {
    const autoCallFunction = async () => {
      try {
        const data = await authService_OrderHistry.getDataByDateAndU_id(
          decodedDate,
          user_id,
          order_id
        );
              ...
              <div className="row">
                <div className="col-12">
                  ...
                  <nav className="breadcrumb-link">
                    ...
                    <span>
                      <Link to={"/admin_order_list"}>Order History</Link>
                    </span>
